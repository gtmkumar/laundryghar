...
> **CANONICAL SOURCE.** The `.sql` files in this folder are the single source of truth for the schema. Any `.md` file (including `PRODUCTION_SPEC.md`) that disagrees is wrong — fix the markdown, not the SQL. This README is generated _from_ the SQL and is safe to read, but the `.sql` files win on any conflict.
...
> **Note on file vs section numbering.** The section numbers below (Section 1–14) describe the _logical_ grouping used in the reference. The _physical_ files above bundle those sections for parallel-agent ownership. Table #1–92 identity is stable across both.
...
**Materialized views:** `mv_daily_store_revenue`, `mv_monthly_franchise_revenue`, `mv_warehouse_throughput`, `mv_customer_ltv`, `mv_rider_performance`
