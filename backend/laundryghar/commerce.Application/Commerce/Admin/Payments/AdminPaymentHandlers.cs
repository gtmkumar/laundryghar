...
namespace commerce.Application.Commerce.Admin.Payments;

// ── Queries ───────────────────────────────────────────────────────────────────

public sealed record GetAdminPaymentsQuery(int Page, int PageSize, Guid? CustomerId = null) : IQuery<PaginatedList<PaymentDto>>;

public sealed class GetAdminPaymentsHandler : IQueryHandler<GetAdminPaymentsQuery, PaginatedList<PaymentDto>>
{
    private readonly ICommerceDbContext _db;
    private readonly ICurrentUser _user;

    public GetAdminPaymentsHandler(ICommerceDbContext db, ICurrentUser user) { _db = db; _user = user; }

    public Task<PaginatedList<PaymentDto>> HandleAsync(GetAdminPaymentsQuery q, CancellationToken ct)
    {
...
public sealed record GetAdminPaymentByIdQuery(Guid Id) : IQuery<PaymentDto?>;

public sealed class GetAdminPaymentByIdHandler : IQueryHandler<GetAdminPaymentByIdQuery, PaymentDto?>
{
    private readonly ICommerceDbContext _db;
    private readonly ICurrentUser _user;

    public GetAdminPaymentByIdHandler(ICommerceDbContext db, ICurrentUser user) { _db = db; _user = user; }

    public async Task<PaymentDto?> HandleAsync(GetAdminPaymentByIdQuery q, CancellationToken ct)
    {
        var brandId = _user.RequireBrandId();
...
public sealed record IssueRefundCommand(IssueRefundRequest Request, Guid? ActorId) : ICommand<PaymentRefundDto>;

public sealed class IssueRefundHandler : ICommandHandler<IssueRefundCommand, PaymentRefundDto>
{
    private readonly ICommerceDbContext _db;
    private readonly ICurrentUser _user;
        ...
        await _db.ExecuteInTransactionAsync(async innerCt =>
        {
            if (req.RefundType == "wallet" && payment.CustomerId.HasValue)
            {
                ...
                var wallet = await _db.WalletAccounts
                    .FirstOrDefaultAsync(w => w.CustomerId == payment.CustomerId.Value && w.BrandId == brandId, innerCt);
            ...
            else
            {
                ...
                if (!string.IsNullOrEmpty(payment.GatewayPaymentId))
                    gatewayRefundId = await _gateway.InitiateRefundAsync(payment.GatewayPaymentId, req.Amount, innerCt);
