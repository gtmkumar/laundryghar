...
namespace commerce.Application.Commerce.Customer.Payments;
...
public sealed class InitiatePaymentHandler : ICommandHandler<InitiatePaymentCommand, PaymentDto>
{
    private readonly ICommerceDbContext _db;
    private readonly IPaymentGateway _gateway;

    public InitiatePaymentHandler(ICommerceDbContext db, IPaymentGateway gateway)
    {
        _db = db;
        _gateway = gateway;
    }

    public async Task<PaymentDto> HandleAsync(InitiatePaymentCommand cmd, CancellationToken ct)
    {
...
public sealed class InitiatePaymentValidator : AbstractValidator<InitiatePaymentRequest>
{
    public InitiatePaymentValidator()
    {
        ...
        RuleFor(x => x.PaymentPurpose).NotEmpty();
    }
}
...
public sealed class VerifyPaymentHandler : ICommandHandler<VerifyPaymentCommand, PaymentDto>
{
    private readonly ICommerceDbContext _db;
    private readonly IPaymentGateway _gateway;

    public VerifyPaymentHandler(ICommerceDbContext db, IPaymentGateway gateway)
    {
        _db = db;
        _gateway = gateway;
    }

    public async Task<PaymentDto> HandleAsync(VerifyPaymentCommand cmd, CancellationToken ct)
    {
        var req = cmd.Request;
