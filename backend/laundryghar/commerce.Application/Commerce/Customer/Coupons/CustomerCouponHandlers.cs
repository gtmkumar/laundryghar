...
namespace commerce.Application.Commerce.Customer.Coupons;

// ── Get applicable coupons ────────────────────────────────────────────────────

public sealed record GetApplicableCouponsQuery(Guid CustomerId, Guid BrandId) : IQuery<List<CouponDto>>;

public sealed class GetApplicableCouponsHandler : IQueryHandler<GetApplicableCouponsQuery, List<CouponDto>>
{
    private readonly ICommerceDbContext _db;

    public GetApplicableCouponsHandler(ICommerceDbContext db) => _db = db;

    public async Task<List<CouponDto>> HandleAsync(GetApplicableCouponsQuery q, CancellationToken ct)
    {
...
) : ICommand<CouponRedemptionDto>;

public sealed class ValidateApplyCouponHandler : ICommandHandler<ValidateApplyCouponCommand, CouponRedemptionDto>
{
    private readonly ICommerceDbContext _db;

    public ValidateApplyCouponHandler(ICommerceDbContext db) => _db = db;

    public async Task<CouponRedemptionDto> HandleAsync(ValidateApplyCouponCommand cmd, CancellationToken ct)
    {
        var req = cmd.Request;
        var now = DateTimeOffset.UtcNow;
...
public sealed class ValidateApplyCouponValidator : AbstractValidator<ValidateCouponRequest>
{
