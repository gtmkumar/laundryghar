...
namespace commerce.Application.Commerce.Customer.Subscriptions;

// ── Query: available plans ────────────────────────────────────────────────────

public sealed record GetActiveSubscriptionPlansQuery(Guid CustomerId, Guid BrandId)
    : IQuery<List<SubscriptionPlanDto>>;

public sealed class GetActiveSubscriptionPlansHandler
    : IQueryHandler<GetActiveSubscriptionPlansQuery, List<SubscriptionPlanDto>>
{
    private readonly ICommerceDbContext _db;

    public GetActiveSubscriptionPlansHandler(ICommerceDbContext db) => _db = db;

    public Task<List<SubscriptionPlanDto>> HandleAsync(GetActiveSubscriptionPlansQuery q, CancellationToken ct)
...
public sealed record GetMySubscriptionsQuery(Guid CustomerId, Guid BrandId)
    : IQuery<List<CustomerSubscriptionDto>>;

public sealed class GetMySubscriptionsHandler
    : IQueryHandler<GetMySubscriptionsQuery, List<CustomerSubscriptionDto>>
{
    private readonly ICommerceDbContext _db;

    public GetMySubscriptionsHandler(ICommerceDbContext db) => _db = db;

    public Task<List<CustomerSubscriptionDto>> HandleAsync(GetMySubscriptionsQuery q, CancellationToken ct)
...
public sealed class SubscribeHandler : ICommandHandler<SubscribeCommand, CustomerSubscriptionDto>
{
    private readonly ICommerceDbContext _db;
    private readonly IPaymentGateway      _gateway;
    private readonly ILogger<SubscribeHandler> _logger;

    public SubscribeHandler(ICommerceDbContext db, IPaymentGateway gateway, ILogger<SubscribeHandler> logger)
    {
        _db      = db;
        _gateway = gateway;
        _logger  = logger;
    ...
    public async Task<CustomerSubscriptionDto> HandleAsync(SubscribeCommand cmd, CancellationToken ct)
    {
...
public sealed class SubscribeCommandValidator : AbstractValidator<SubscribeRequest>
{
    public SubscribeCommandValidator()
    {
        RuleFor(x => x.PlanId).NotEmpty();
        RuleFor(x => x.MandateType).NotEmpty()
        ...
        RuleFor(x => x.MaxMandateAmount).GreaterThan(0);
        RuleFor(x => x.UpiVpa)
            .NotEmpty()
            .When(x => x.MandateType == "upi_autopay")
...
public sealed class CancelSubscriptionHandler : ICommandHandler<CancelSubscriptionCommand, CustomerSubscriptionDto?>
{
    private readonly ICommerceDbContext _db;

    public CancelSubscriptionHandler(ICommerceDbContext db) => _db = db;

    public async Task<CustomerSubscriptionDto?> HandleAsync(CancelSubscriptionCommand cmd, CancellationToken ct)
    {
