...
// evidence before the laundry takes possession. This record lets warehouse staff
// see the pre-pickup state and protects the brand from liability disputes.
//
// Storage design: garment_inspections requires garment_id NOT NULL, but
// individual garment rows are created by warehouse intake AFTER pickup — they
// don't exist yet. We therefore store the evidence on the delivery_assignment
// itself via the metadata jsonb column:
//
//   {
//     "pickup_inspection": {
//       "recordedAt": "<ISO>",
//       "frontPhotoKey": "<storage-key>",
//       "backPhotoKey":  "<storage-key> | null",
//       "conditions": { "stains": bool, "tears": bool, "missingButtons": bool },
//       "notes": "<text | null>"
//     }
//   }
//
// Warehouse intake can later link these photo keys to GarmentInspectionPhoto
// rows once real garment tags are assigned. The endpoint is additive: calling
// it a second time (retry) replaces the inspection block — same net result.
// ──────────────────────────────────────────────────────────────────────────────
...
public sealed record SubmitPickupInspectionCommand(
    ...
    string?             Notes) : IRequest<RiderTaskResult>;

public sealed class SubmitPickupInspectionHandler
    : IRequestHandler<SubmitPickupInspectionCommand, RiderTaskResult>
{
    private readonly LaundryGharDbContext  _db;
    private readonly IFileStorageProvider _storage;

    public SubmitPickupInspectionHandler(LaundryGharDbContext db, IFileStorageProvider storage)
    {
        _db      = db;
        _storage = storage;
    }

    public async Task<RiderTaskResult> Handle(
        SubmitPickupInspectionCommand cmd, CancellationToken ct)
    {
    ...
    /// We use XorShift over the bytes — it's not RFC 4122 v5, but it's stable and
    /// non-colliding within the domain of {assignmentId + suffix}.
    /// </summary>
