...
public sealed record GetPayoutRequestsQuery(string? Status) : IRequest<IReadOnlyList<PayoutRequestAdminDto>>;

public sealed class GetPayoutRequestsHandler : IRequestHandler<GetPayoutRequestsQuery, IReadOnlyList<PayoutRequestAdminDto>>
{
    private readonly LaundryGharDbContext _db;
    public GetPayoutRequestsHandler(LaundryGharDbContext db) => _db = db;

    public async Task<IReadOnlyList<PayoutRequestAdminDto>> Handle(GetPayoutRequestsQuery q, CancellationToken ct)
    {
...
public sealed record ReviewPayoutRequestCommand(Guid RequestId, bool Approve, string? Reason, Guid? ActorId)
    : IRequest<PayoutRequestAdminDto?>;

public sealed class ReviewPayoutRequestHandler : IRequestHandler<ReviewPayoutRequestCommand, PayoutRequestAdminDto?>
{
    private readonly LaundryGharDbContext _db;
    public ReviewPayoutRequestHandler(LaundryGharDbContext db) => _db = db;

    public async Task<PayoutRequestAdminDto?> Handle(ReviewPayoutRequestCommand cmd, CancellationToken ct)
    {
        var req = await _db.RiderPayoutRequests.FirstOrDefaultAsync(p => p.Id == cmd.RequestId, ct);
        ...
        req.Status = cmd.Approve ? RiderPayoutRequestStatus.Approved : RiderPayoutRequestStatus.Rejected;
        req.RejectionReason = cmd.Approve ? null : cmd.Reason;
        req.ReviewedBy = cmd.ActorId;
        req.ReviewedAt = now;
        req.UpdatedAt = now;
        req.UpdatedBy = cmd.ActorId;
...
public sealed class MarkPayoutPaidHandler : IRequestHandler<MarkPayoutPaidCommand, PayoutRequestAdminDto?>
{
    private readonly LaundryGharDbContext _db;
    public MarkPayoutPaidHandler(LaundryGharDbContext db) => _db = db;

    public async Task<PayoutRequestAdminDto?> Handle(MarkPayoutPaidCommand cmd, CancellationToken ct)
    {
        var req = await _db.RiderPayoutRequests.FirstOrDefaultAsync(p => p.Id == cmd.RequestId, ct);
        if (req is null) return null;
        ...
        req.PaymentReference = cmd.Reference;
        req.PaidBy = cmd.ActorId;
        req.PaidAt = now;
        req.UpdatedAt = now;
        req.UpdatedBy = cmd.ActorId;

        await PostCashOutAsync(req, now, cmd.ActorId, ct);
        await _db.SaveChangesAsync(ct);
