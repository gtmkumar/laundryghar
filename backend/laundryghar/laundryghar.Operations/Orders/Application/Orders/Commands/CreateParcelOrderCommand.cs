...
public sealed class CreateParcelOrderHandler : IRequestHandler<CreateParcelOrderCommand, OrderDto>
{
    private readonly LaundryGharDbContext _db;
    private readonly OrdersSettings _settings;
    private readonly IFieldCipher _cipher;

    public CreateParcelOrderHandler(LaundryGharDbContext db, IOptions<OrdersSettings> opts, IFieldCipher cipher)
    {
        _db = db;
    ...
    public async Task<OrderDto> Handle(CreateParcelOrderCommand cmd, CancellationToken ct)
    {
        ...
        var orderNumber = await _db.Database
            .SqlQuery<string>($"SELECT order_lifecycle.next_order_number({brandId}, {store.Id}, {store.Code}, {now.Year}) AS \"Value\"")
            .SingleAsync(ct);
