...
public sealed record GetFareQuoteQuery(Guid CustomerId, Guid BrandId, FareQuoteRequest Request)
    : IRequest<FareQuoteDto>;

public sealed class GetFareQuoteHandler : IRequestHandler<GetFareQuoteQuery, FareQuoteDto>
{
    private readonly LaundryGharDbContext _db;
    private readonly IFieldCipher _cipher;

    public GetFareQuoteHandler(LaundryGharDbContext db, IFieldCipher cipher)
    {
        _db = db;
        _cipher = cipher;
    }

    public async Task<FareQuoteDto> Handle(GetFareQuoteQuery q, CancellationToken ct)
    {
        var req = q.Request;

        if (req.VehicleTier is not null && !VehicleTier.IsValid(req.VehicleTier))
            throw new BusinessRuleException(
                $"VehicleTier must be one of: {string.Join(", ", SharedDataModel.Enums.VehicleTier.All)}.");
