...
namespace laundryghar.Orders.Application.Invoices.Commands;
...
/// </summary>
public sealed record GenerateInvoiceCommand(Guid OrderId, Guid? ActorId) : IRequest<InvoiceDto>;

public sealed class GenerateInvoiceHandler : IRequestHandler<GenerateInvoiceCommand, InvoiceDto>
{
    ...
    public GenerateInvoiceHandler(LaundryGharDbContext db, ICurrentUser user)
    {
        _db   = db;
        _user = user;
    }

    public async Task<InvoiceDto> Handle(GenerateInvoiceCommand cmd, CancellationToken ct)
    {
