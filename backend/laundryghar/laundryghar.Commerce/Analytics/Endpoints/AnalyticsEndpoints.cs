...
public static class AnalyticsEndpoints
{
    public static WebApplication MapAnalyticsEndpoints(this WebApplication app)
    {
            ...
            [FromQuery] int pageSize = 20) =>
        {
            ...
            // mv_rider_performance emits NULL for the four aggregate columns when a
            // rider has no qualifying rows for the day. The entity is nullable to avoid
            // an InvalidCastException on materialization; the response coalesces null→0
            // so the JSON shape stays numeric (backward compatible with existing clients).
            var dto = paged.Map(RiderPerformanceResponse.From);
            return Results.Ok(new PaginatedListResponse<RiderPerformanceResponse> { Status = true, Data = dto });
        }).RequireAuthorization("permission:analytics.read");
