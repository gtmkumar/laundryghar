...
/// <see cref="LaundryGharDbContext"/> scope. The <see cref="GatewaySettingsCache"/> is
/// Singleton — it holds the TTL cache across requests.
/// </summary>
internal sealed class SettingsFirstPaymentGateway : IPaymentGateway
{
    ...
    private RazorpayPaymentGateway Build(string keyId, string keySecret, string? webhookSecret)
    {
        ...
        return new RazorpayPaymentGateway(
            ...
            _logger as ILogger<RazorpayPaymentGateway>
                ?? Microsoft.Extensions.Logging.Abstractions.NullLogger<RazorpayPaymentGateway>.Instance);
    }
}
