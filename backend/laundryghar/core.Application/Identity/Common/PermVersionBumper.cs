...
public static class PermVersionBumper
{
    ...
    /// <summary>Bump brand-scoped members of a brand — e.g. after an entitlement change.
    /// Franchise/store-scoped members refresh on their next natural token refresh.</summary>
    public static async Task BumpBrandMembersAsync(ICoreDbContext db, Guid brandId, CancellationToken ct)
    {
        var ids = await db.UserScopeMemberships.AsNoTracking()
            .Where(m => m.RevokedAt == null && m.ScopeType == ScopeType.Brand && m.ScopeId == brandId)
            .Select(m => m.UserId).Distinct().ToListAsync(ct);
        await BumpUsersAsync(db, ids, ct);
    }
}
