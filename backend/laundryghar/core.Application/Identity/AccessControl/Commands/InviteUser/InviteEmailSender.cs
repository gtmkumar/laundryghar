...
/// brand's provisioning mode. Shared by the initial invite and the resend-invite flow. Best-effort:
...
internal static class InviteEmailSender
{
        ...
        ICurrentUser actor, Guid userId, string? email, string name, CancellationToken ct)
    {
        ...
        try
        {
            var mode = await SettingsStore.LoadProvisioningModeAsync(db, actor.BrandId, ct);
            if (mode == "self_service")
            {
                ...
                var baseUrl = (await SettingsStore.LoadAdminBaseUrlAsync(db, actor.BrandId, ct)).TrimEnd('/');
                ...
                await mailer.SendAsync(actor.BrandId, email, subject, html, ct);
            }
            else
            {
