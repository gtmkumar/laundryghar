...
public sealed class SettingsMailer : ISettingsMailer
{
    ...
    public SettingsMailer(LaundryGharDbContext db, ILogger<SettingsMailer> log)
    {
        _db = db;
        _log = log;
    }

    public async Task<EmailSettings?> LoadAsync(Guid? brandId, CancellationToken ct = default)
    {
        // The RLS connection interceptor already scopes this query to the request's
        // brand (or all brands for a platform admin via bypass), so an unfiltered
        ...
        var row = await query.OrderBy(s => s.BrandId == null).FirstOrDefaultAsync(ct);
    ...
    public async Task<bool> SendAsync(Guid? brandId, string to, string subject, string htmlBody, CancellationToken ct = default)
    {
        var cfg = await LoadAsync(brandId, ct);
    ...
    public async Task<(bool ok, string? error)> TestAsync(EmailSettings cfg, string to, CancellationToken ct = default)
    {
        ...
        try
        {
            await DeliverAsync(cfg, to,
                "Laundry Ghar — SMTP test",
                "<p>This is a test email from your Laundry Ghar admin console.</p>" +
    ...
    private static async Task DeliverAsync(EmailSettings cfg, string to, string subject, string htmlBody, CancellationToken ct)
    {
        ...
        msg.From.Add(new MailboxAddress(cfg.FromName ?? "Laundry Ghar", cfg.FromEmail));
