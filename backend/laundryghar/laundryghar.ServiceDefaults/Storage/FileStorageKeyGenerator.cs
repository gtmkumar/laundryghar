...
public static class FileStorageKeyGenerator
{
    ...
    // This permits sub-area paths like "docs/kyc" while rejecting absolute paths like "/etc/passwd".
    ...
    /// </summary>
    /// <param name="brandId">Tenant brand — forms the first path segment.</param>
    /// <param name="area">Purpose label (e.g. <c>inspections</c>, <c>proof</c>).
    ...
    public static string Generate(Guid brandId, string area, string contentType)
    {
        ...
        // brandId without hyphens keeps the key compact and unambiguous
        var brand = brandId.ToString("N");

        return $"{brand}/{area}/{id}.{ext}";
    }
