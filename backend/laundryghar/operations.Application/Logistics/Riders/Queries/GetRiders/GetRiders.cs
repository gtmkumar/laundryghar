...
namespace operations.Application.Logistics.Riders.Queries.GetRiders;
...
public sealed record GetRidersQuery(
    ...
    string? KycStatus,
    string? Sort)
    : IQuery<PaginatedList<RiderDto>>;

public sealed class GetRidersHandler : IQueryHandler<GetRidersQuery, PaginatedList<RiderDto>>
{
    private readonly IOperationsDbContext _db;
    private readonly ICurrentUser _user;
    public GetRidersHandler(IOperationsDbContext db, ICurrentUser user) { _db = db; _user = user; }

    public async Task<PaginatedList<RiderDto>> HandleAsync(GetRidersQuery query, CancellationToken cancellationToken)
    {
        var ct      = cancellationToken;
        var q       = query;
        var brandId = _user.RequireBrandId();
        var dbQuery = _db.Riders.Where(r => r.BrandId == brandId);
        ...
        if (_user.FranchiseId is Guid actorFid)
            dbQuery = dbQuery.Where(r => r.FranchiseId == actorFid);
        else if (q.FranchiseId.HasValue)
            dbQuery = dbQuery.Where(r => r.FranchiseId == q.FranchiseId.Value);

        if (!string.IsNullOrEmpty(q.Status))
            dbQuery = dbQuery.Where(r => r.Status == q.Status);

        if (!string.IsNullOrEmpty(q.KycStatus))
            dbQuery = dbQuery.Where(r => r.KycStatus == q.KycStatus);
        ...
        {
            "kyc"      => descending ? dbQuery.OrderByDescending(r => r.KycStatus)   : dbQuery.OrderBy(r => r.KycStatus),
            "status"   => descending ? dbQuery.OrderByDescending(r => r.Status)      : dbQuery.OrderBy(r => r.Status),
            "name"     => descending ? dbQuery.OrderByDescending(r => r.RiderCode)   : dbQuery.OrderBy(r => r.RiderCode),
            "franchise"=> descending ? dbQuery.OrderByDescending(r => r.FranchiseId) : dbQuery.OrderBy(r => r.FranchiseId),
            _          => dbQuery.OrderByDescending(r => r.CreatedAt), // "created" + fallback
        };
