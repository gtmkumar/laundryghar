...
namespace operations.Application.Logistics.RiderSelf.Commands.UploadProofPhoto;
...
public sealed record UploadProofPhotoCommand(
    ...
    Guid      UserId,
    Guid      BrandId,
    IFormFile File) : ICommand<RiderTaskResult>;

public sealed class UploadProofPhotoHandler : ICommandHandler<UploadProofPhotoCommand, RiderTaskResult>
{
    private readonly IOperationsDbContext  _db;
    private readonly IFileStorageProvider _storage;

    public UploadProofPhotoHandler(IOperationsDbContext db, IFileStorageProvider storage)
    {
        _db      = db;
        _storage = storage;
    }

    public async Task<RiderTaskResult> HandleAsync(UploadProofPhotoCommand command, CancellationToken cancellationToken)
    {
