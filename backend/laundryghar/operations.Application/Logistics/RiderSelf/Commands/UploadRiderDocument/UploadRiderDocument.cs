...
namespace operations.Application.Logistics.RiderSelf.Commands.UploadRiderDocument;
...
public sealed record UploadRiderDocumentCommand(
    Guid UserId, Guid BrandId, string DocType, IFormFile File) : ICommand<RiderDocumentDto>;

public sealed class UploadRiderDocumentHandler : ICommandHandler<UploadRiderDocumentCommand, RiderDocumentDto>
{
    private readonly IOperationsDbContext _db;
    private readonly IFileStorageProvider _storage;

    public UploadRiderDocumentHandler(IOperationsDbContext db, IFileStorageProvider storage)
    {
        _db = db;
        _storage = storage;
    }

    public async Task<RiderDocumentDto> HandleAsync(UploadRiderDocumentCommand command, CancellationToken cancellationToken)
    {
...
public sealed class UploadRiderDocumentValidator : AbstractValidator<UploadRiderDocumentCommand>
{
    ...
    public UploadRiderDocumentValidator()
    {
        ...
        RuleFor(x => x.File.ContentType).Must(c => AllowedMime.Contains(c))
...
public sealed record GetMyRiderVerificationQuery(Guid UserId, Guid BrandId) : IQuery<RiderVerificationView?>;

public sealed class GetMyRiderVerificationHandler : IQueryHandler<GetMyRiderVerificationQuery, RiderVerificationView?>
{
    private readonly IOperationsDbContext _db;
    public GetMyRiderVerificationHandler(IOperationsDbContext db) => _db = db;

    public async Task<RiderVerificationView?> HandleAsync(GetMyRiderVerificationQuery query, CancellationToken cancellationToken)
    {
        var ct = cancellationToken;
        var rider = await _db.Riders.AsNoTracking()
            .FirstOrDefaultAsync(r => r.UserId == query.UserId && r.BrandId == query.BrandId, ct);
