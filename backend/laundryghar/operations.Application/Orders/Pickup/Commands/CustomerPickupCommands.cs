...
public sealed class ValidateCouponForPickupHandler
    : IQueryHandler<ValidateCouponForPickupQuery, CouponPreviewResult>
{
    private readonly IOperationsDbContext _db;

    public ValidateCouponForPickupHandler(IOperationsDbContext db) => _db = db;

    public async Task<CouponPreviewResult> HandleAsync(ValidateCouponForPickupQuery q, CancellationToken ct)
    {
...
public sealed class ReschedulePickupHandler : ICommandHandler<ReschedulePickupCommand, PickupRequestDto?>
{
    ...
    public ReschedulePickupHandler(IOperationsDbContext db, ILogger<ReschedulePickupHandler> logger)
    {
        _db     = db;
        _logger = logger;
    }

    public async Task<PickupRequestDto?> HandleAsync(ReschedulePickupCommand cmd, CancellationToken ct)
    {
        var req = cmd.Request;
        ...
        await _db.ExecuteInTransactionAsync(async innerCt =>
        {
            ...
            if (cmd.Request.NewSlotId.HasValue)
            {
                ...
                var newSlot = await _db.DeliverySlots
                    .FirstOrDefaultAsync(s => s.Id == cmd.Request.NewSlotId.Value, innerCt);
...
public sealed record CancelPickupByCustomerCommand(
    ...
    string? Reason,
    Guid? ActorId
) : ICommand<CancelPickupResult>;

public sealed class CancelPickupByCustomerHandler : ICommandHandler<CancelPickupByCustomerCommand, CancelPickupResult>
{
    ...
    public CancelPickupByCustomerHandler(IOperationsDbContext db, ILogger<CancelPickupByCustomerHandler> logger)
    {
        _db = db;
        _logger = logger;
    }
    ...
    public async Task<CancelPickupResult> HandleAsync(CancelPickupByCustomerCommand cmd, CancellationToken ct)
    {
        ...
        await _db.ExecuteInTransactionAsync(async innerCt =>
        {
            ...
            foreach (var a in assignments)
            {
                ...
                await RiderLoad.DecrementAsync(_db, a.RiderId, innerCt);
            }
