...
public sealed record CreatePickupRequestAdminCommand(
    ...
    Guid CustomerId,
    Guid? ActorId
) : ICommand<PickupRequestDto>;

public sealed class CreatePickupRequestAdminHandler
    : ICommandHandler<CreatePickupRequestAdminCommand, PickupRequestDto>
{
    private readonly IOperationsDbContext _db;
    private readonly ICurrentUser _user;

    public CreatePickupRequestAdminHandler(IOperationsDbContext db, ICurrentUser user)
    {
        _db = db;
        _user = user;
    }

    public async Task<PickupRequestDto> HandleAsync(CreatePickupRequestAdminCommand cmd, CancellationToken ct)
    {
...
public sealed class AssignPickupHandler : ICommandHandler<AssignPickupCommand, DeliveryAssignmentDto?>
{
    private readonly IOperationsDbContext _db;
    private readonly ICurrentUser _user;

    public AssignPickupHandler(IOperationsDbContext db, ICurrentUser user)
    {
        _db = db;
        _user = user;
    }
    ...
    public async Task<DeliveryAssignmentDto?> HandleAsync(AssignPickupCommand cmd, CancellationToken ct)
    {
        var brandId = _user.RequireBrandId();
        ...
        await RiderLoad.IncrementAsync(_db, cmd.Request.RiderId, ct);
...
) : ICommand<CustomerSchedulePickupResult>;
...
public sealed class CustomerSchedulePickupHandler
    : ICommandHandler<CustomerSchedulePickupCommand, CustomerSchedulePickupResult>
{
    ...
    public async Task<CustomerSchedulePickupResult> HandleAsync(CustomerSchedulePickupCommand cmd, CancellationToken ct)
    {
        var req    = cmd.Request;
        ...
        PickupRequestDto pickupDto = default!;
        try
        {
            await _db.ExecuteInTransactionAsync(async innerCt =>
            {
                ...
                if (req.SlotId.HasValue)
                {
                    var updated = await _db.ExecuteSqlInterpolatedAsync(
                        $"""
                    ...
                    var slot = await _db.DeliverySlots
                        .FirstOrDefaultAsync(s => s.Id == req.SlotId.Value, innerCt);
                ...
                var dto = await CreatePickupRequestAdminHandler.CreatePickup(
                    ...
                    req: req, actorId: cmd.CustomerId, ct: innerCt,
                    idempotencyKey: normKey,
                    source: source);
                ...
                if (req.SlotId.HasValue)
                {
                    var booking = await _db.DeliverySlotBookings
                        .FirstOrDefaultAsync(b => b.SlotId          == req.SlotId.Value
                                               ...
                                               && b.PickupRequestId == null, innerCt);
...
public sealed class RejectPickupHandler : ICommandHandler<RejectPickupCommand, PickupRequestDto?>
{
    ...
    public RejectPickupHandler(IOperationsDbContext db, ICurrentUser user)
    {
        _db = db;
        _user = user;
    }

    public async Task<PickupRequestDto?> HandleAsync(RejectPickupCommand cmd, CancellationToken ct)
    {
        var brandId = _user.RequireBrandId();
