...
public sealed record CreateDeliverySlotCommand(CreateDeliverySlotRequest Request, Guid? ActorId)
    : ICommand<DeliverySlotDto>;

public sealed class CreateDeliverySlotHandler : ICommandHandler<CreateDeliverySlotCommand, DeliverySlotDto>
{
    private readonly IOperationsDbContext _db;
    private readonly ICurrentUser _user;

    public CreateDeliverySlotHandler(IOperationsDbContext db, ICurrentUser user)
    {
        _db   = db;
        _user = user;
    }

    public async Task<DeliverySlotDto> HandleAsync(CreateDeliverySlotCommand cmd, CancellationToken ct)
    {
...
public sealed class UpdateDeliverySlotHandler : ICommandHandler<UpdateDeliverySlotCommand, DeliverySlotDto?>
{
    private readonly IOperationsDbContext _db;
    private readonly ICurrentUser _user;

    public UpdateDeliverySlotHandler(IOperationsDbContext db, ICurrentUser user)
    {
        _db   = db;
        _user = user;
    }

    public async Task<DeliverySlotDto?> HandleAsync(UpdateDeliverySlotCommand cmd, CancellationToken ct)
    {
        var brandId = _user.RequireBrandId();
