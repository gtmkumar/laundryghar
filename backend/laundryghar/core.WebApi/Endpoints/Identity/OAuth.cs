...
///
/// This endpoint is host code: it injects the concrete <see cref="LaundryGharDbContext"/>
/// directly and writes inline LINQ/SQL (NOT routed through CQRS) so the OAuth/PKCE protocol
/// logic stays in one auditable place. The customer OTP send/verify steps DO delegate to the
/// shared E7 CQRS commands so lockout / salted-hash / rate-limit behaviour is reused.
///
/// Endpoints registered here (all .AllowAnonymous()):
///   GET  /.well-known/oauth-authorization-server  — RFC 8414 server metadata ("auth")
///   POST /oauth/register                           — RFC 7591 dynamic client registration ("oauth_register")
///   GET  /oauth/authorize                          — consent page (HTML/JS) ("auth")
///   POST /oauth/authorize/otp/send                 — backing: OTP send for the authorize flow ("auth")
///   POST /oauth/authorize/approve                  — backing: OTP verify → issue auth code ("auth")
///   POST /oauth/token                              — token exchange (authorization_code + refresh_token) ("auth")
///
/// Security invariants:
...
///
/// Routes live at mixed roots (/.well-known/*, /oauth/*), so the group prefix is empty.
/// </summary>
public class OAuth : IEndpointGroup
{
    public static string? RoutePrefix => "";

    public static void Map(RouteGroupBuilder group)
    {
        group.WithTags("OAuth");

        // ── RFC 8414 — Authorization Server Metadata ─────────────────────────
        // Served at well-known location; static metadata (no DB).
        group.MapGet("/.well-known/oauth-authorization-server",
        ...
        // Reuses the E7 CustomerOtpSendCommand + rate-limiting/cooldown logic.
        // Brand resolved from DefaultBrandCode config (no X-Brand-Id header / no body brandCode),
        // mirroring the public OTP endpoint with no brand inputs.
        group.MapPost("/oauth/authorize/otp/send",
                   ...
                   CancellationToken ct) =>
            {
                ...
                // No header/body brand for the OAuth flow → handler resolves the default brand.
                var result = await dispatcher.SendAsync(
                    new CustomerOtpSendCommand(req.Phone, null, null, ip, ua), ct);
        ...
        // Verifies OTP (via the E7 command), resolves the customer, validates
        // client + redirect_uri, then issues a single-use PKCE authorization code.
        group.MapPost("/oauth/authorize/approve",
                   ...
                   CancellationToken ct) =>
            {
                ...
                //    This MUST match the brand the E7 verify command resolves below
                //    (both fall back to CustomerAuth:DefaultBrandCode → "LG-MAIN").
                Guid brandId;
                try
                {
                ...
                // 4. Verify OTP via the E7 command (inherits lockout / salted-hash / rate-limit).
                //    No header/body brand → handler resolves the same default brand as step 3.
                var ip = ctx.Connection.RemoteIpAddress?.ToString();
                var ua = ctx.Request.Headers.UserAgent.ToString();
                ...
                try
                {
                    await dispatcher.SendAsync(
                        new CustomerOtpVerifyCommand(req.Phone, req.Code, null, null, ip, ua), ct);
                }
        ...
        group.MapPost("/oauth/token",
                   ...
                   CancellationToken ct) =>
            {
                ...
                if (grantType == "authorization_code")
                    return await HandleAuthorizationCodeGrantAsync(form, db, jwt, refreshTokens, jwtOptions, logger, ctx, ct);
    ...
    /// Matches the logic the E7 customer-OTP handlers use when no header/body brand is provided.
    /// </summary>
