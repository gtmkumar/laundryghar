...
/// <summary>ICurrentUser backed by HttpContext JWT claims.</summary>
public sealed class HttpContextCurrentUser : ICurrentUser
{
    ...
    public string? Email       => Claim(System.IdentityModel.Tokens.Jwt.JwtRegisteredClaimNames.Email);
    ...
    /// <summary>
    /// Returns the effective brand for write operations.
    /// Platform admins may pass X-Brand-Id (captured in Items["brand_id_override"] by TenantResolutionMiddleware).
    /// Throws if no brand can be resolved.
    /// </summary>
    public Guid RequireBrandId()
    {
        // Try X-Brand-Id override (platform admin workflow)
        ...
        // Fall back to JWT brand_id claim
    ...
    private Guid?   ParseGuid(string type) =>
        Guid.TryParse(Claim(type), out var g) ? g : null;
}
