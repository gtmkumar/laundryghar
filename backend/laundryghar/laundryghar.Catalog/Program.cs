...
builder.Services.AddAuthentication(JwtBearerDefaults.AuthenticationScheme)
    .AddJwtBearer(opts =>
    {
        opts.TokenValidationParameters = new TokenValidationParameters
        {
            ...
            // M1: pin allowed algorithm — rejects any token signed with a different algorithm
            // (e.g. RS256 confusion attacks where a public key is used as an HMAC secret).
            ValidAlgorithms          = new[] { SecurityAlgorithms.HmacSha256 }
        };
    });

// ─── Authorization (permission policy provider + CustomerOnly) ─────────────
...
if (runSeed)
{
    ...
    var seeder = ActivatorUtilities.CreateInstance<CatalogSeeder>(scope.ServiceProvider, seedDb);
